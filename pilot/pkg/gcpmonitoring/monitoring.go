...
func IncrementConfigEventMeasure(t, operation string) {
	ctx, err := tag.New(context.Background(), tag.Insert(operationKey, operation), tag.Insert(typeKey, t))
	if err != nil {
		return
	}
	stats.Record(ctx, configEventMeasure.M(1))
}
...
func IncrementConfigValidationMeasuare(t string, success bool) {
	...
	stats.Record(ctx, configValidationMeasuare.M(1))
}
...
func IncrementConfigPushMeasuare(t string, success bool) {
	...
	stats.Record(ctx, configPushMeasuare.M(1))
}

func IncrementPilotReject(xdsType string) {
	ctx, err := tag.New(context.Background(), tag.Insert(typeKey, xdsType))
	if err != nil {
		return
	}
	stats.Record(ctx, rejectedConfigMeasuare.M(1))
}

func RecordPilotConfigConvergence(value float64) {
	stats.Record(context.Background(), configConvergenceMeasuare.M(value))
}

func RecordXdsClients(value int) {
