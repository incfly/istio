...
func TestGCPMonitoringSidecarInjection(t *testing.T) {
	...
	var cases = []struct {
		...
		{"totalSuccessfulInjections", incrementSuccessInjection, &view.Row{
			Tags: []tag.Tag{{Key: tag.MustNewKey("success"), Value: "true"}}, Data: &view.SumData{1.0}}},
		{"totalFailedInjections", incrementFailedInjection, &view.Row{
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := retry.UntilSuccess(func() error {
				...
				return fmt.Errorf("metrics %v does not have expected values, want %+v", tt.name, tt.wantVal)
			}); err != nil {
